		...
		<view :class="lotusAddressData.visible?'lotus-address-box':'lotus-address-box lotus-address-box-out'">
			...
			<view class="lotus-address-picker-box">
				...
				<scroll-view scroll-y :scroll-into-view="'pid'+pChoseIndex" class="lotus-address-picker-box-item">
					<view @tap="clickPicker(0,pIndex,pItem);" :id="'pid'+pIndex" :class="pIndex === pChoseIndex?'lotus-address-picker lotus-address-picker2':'lotus-address-picker'"  v-for="(pItem,pIndex) in province" :key="pIndex">{{pItem.name}}</view>
				</scroll-view>
				<!--市-->
				<scroll-view scroll-y :scroll-into-view="'cid'+cChoseIndex" class="lotus-address-picker-box-item">
					<view @tap="clickPicker(1,cIndex,cItem);" :id="'cid'+cIndex" :class="cIndex === cChoseIndex?'lotus-address-picker lotus-address-picker2':'lotus-address-picker'" v-for="(cItem,cIndex) in city" :key="cIndex">{{cItem.name}}</view>
				</scroll-view>
				<!--区-->
				<scroll-view scroll-y :scroll-into-view="'tid'+tChoseIndex" class="lotus-address-picker-box-item" v-if="lotusAddressData.showt" >
					<view @tap="clickPicker(2,tIndex,tItem);" :id="'tid'+tIndex" :class="tIndex === tChoseIndex?'lotus-address-picker lotus-address-picker2':'lotus-address-picker'" v-for="(tItem,tIndex) in town" :key="tIndex">{{tItem.name}}</view>
				</scroll-view>
				<!--区END-->
			</view>
		</view>
...
<script>
	import {lotusAddressJson} from  "./loutusAddress.js";
	export default {
		...
		methods:{
			//取消
			cancelPicker(){
				const provinceCode = this.getTarId(this.provinceName,'p');
				const cityCode = this.getTarId(this.cityName,'c');
				const townCode = this.getTarId(this.townName,'t');
				this.visible = false;
				this.$emit("choseVal",{
			...
			chosedVal() {
				this.type = 1;
				let showt = this._props.lotusAddressData.showt;
				let v = showt ? 'id' : 'code';
				let provinceCode = this.myGetTar(this.provinceName,'p',v);
				let cityCode = this.myGetTar(this.cityName,'c',v);
				const townCode = this.myGetTar(this.townName,'t','id');
				this.visible = false;
				let isChose = 0;
				//已选省市区 isChose = 1
			...
			getTarCode(name,type){
			    let id = 0;
			    lotusAddressJson.map((item,index)=>{
			        if(item.name === name){
			            id = item.code ? item.code : item.value;
			        }
			    });
			    return id;
			},
			myGetTar(name,type,v){
			    let id = 0;
				let data = null;
				if(type == 'p'){
					data = this.province
			...
			getTownArr(parentId){
			    let town = [];
			    lotusAddressJson.map((item,index)=>{
			        if(index>30&&item.parent === parentId){
			            town.push(item);
			        }
			    });
			    return town;
			...
			//获取已选省市区
			getChosedData(){
				//const pid = this.myGetTar(this.provinceName,'p','id');
