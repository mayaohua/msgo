...
<script type="text/javascript">
    new Vue({
        ...
        created() {
			this.init(false)
			let user_key = window.sessionStorage.getItem('user_key')
            if(!user_key){return;}
		...
		methods: {
			init(data){
				this.api.getDataList('/bill/vip',data, res => {
			...
			inputClear(){
			  ...
			  onInput(value){
			    ...
			    if(value.length === 11){
					...
					this.api.getDataList('/bill/vip',{mobile:value}, res => {
                        if(res.code != 0){
                            this.input.phone = '';
                            this.list = this.tempList;
                        }else{
			...
			onSellPut(event){
                ...
                st_obj.desc = st_obj.desc.replace(/\[/g,'<i style="color: red;font-style:normal;">').replace(/\]/g,'</i>') ? st_obj.desc : '';
			    ...
			    <p ${p_sty}><span ${t_sty}>原价</span><span ${s_sty}>${st_obj.yj}元</span></p>
