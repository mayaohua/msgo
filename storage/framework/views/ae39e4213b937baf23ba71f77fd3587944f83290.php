                ...
                <div class="ui-content" v-for="(caseitem,caseindex) in item.billcases" :key="caseindex">
                    ...
                    <div class="ui-items">
                        ...
                        @click="onSellPut" 
                            ...
                            :data-package="billitem.sell_data.facePrice"
                            :data-yh_tips="billitem.yh_tips"
                            :data-id="billitem.id">
                            <div class="ui-cont">
                                <div class="small-desc">{{billitem.yh_tips}}</div>
                                <div style="margin:5px 0;"><span class="text-red">面值：{{billitem.sell_data.facePrice}}元</span>
                                </div>
...
<script type="text/javascript">
    new Vue({
		...
		methods: {
			init(data){
				this.api.getDataList('/bill/call', data, res => {
			...
			inputClear(){
			  ...
			  onInput(value){
			    ...
			    if(value.length === 11){
					...
					this.api.getDataList('/bill/call',{mobile:value}, res => {
                        if(res.code != 0){
                            this.input.phone = '';
                            this.list = this.tempList;
                        }else{
			...
			onSellPut(event){
			    ...
			    var str = `
			    <p ${p_sty}><span ${t_sty}>号码</span><span ${s_sty}>${this.input.phone}</span></p>
			    <p ${p_sty}><span ${t_sty}>套餐</span><span ${s_sty}>${st_obj.cc}</span></p>
			    <p ${p_sty}><span ${t_sty}>预计</span><span ${s_sty}>${st_obj.yh_tips}</span></p>
			    
			    <p ${p_sty}><span ${t_sty}>面值</span><span ${s_sty}>${st_obj.package}元</span></p>
