        ...
        <el-card class="box-card">
            ...
            <div class="text item">
                ...
                <el-table :data="tableData.data" style="width: 100%">
                            ...
                            <template v-for="item in props.row.status_items">
                                <el-form  label-position="left" inline class="demo-table-expand">
                                    ...
                                    <el-form-item  style="margin-bottom:0" label="状态说明：">
                                        <span style="color:red">@{{ item.status_desc }}</span>
                                    </el-form-item>
                    ...
                    <el-table-column prop="order_from" label="下单途径">
                    <template slot-scope="scope">@{{scope.row.order_from == 'official'?'公众号':'小程序'}}</template>
                    </el-table-column>
                    ...
                    <el-table-column prop="bill_fail_msg" label="错误原因">
                    </el-table-column>
                    <el-table-column prop="bill_status" label="下单状态">
                        <template slot-scope="scope">
                            <el-tag type="success" v-if="scope.row.bill_status == 0" disable-transitions>等待付款</el-tag>
                            <el-tag type="success" v-if="scope.row.bill_status == 1" disable-transitions>支付成功</el-tag>
                            <el-tag type="success" v-if="scope.row.bill_status == 2" disable-transitions>支付失败</el-tag>
                            <el-tag type="success" v-if="scope.row.bill_status == 3" disable-transitions>充值成功</el-tag>
                            <el-tag type="success" v-if="scope.row.bill_status == 4" disable-transitions>充值失败</el-tag>
                            <el-tag type="success" v-if="scope.row.bill_status == 5" disable-transitions>话费充值中</el-tag>
                            <el-tag type="success" v-if="scope.row.bill_status == 6" disable-transitions>退款中</el-tag>
                            <el-tag type="success" v-if="scope.row.bill_status == 7" disable-transitions>退款成功</el-tag>
                            <el-tag type="success" v-if="scope.row.bill_status == 8" disable-transitions>退款失败</el-tag>
                            <el-tag type="success" v-if="scope.row.bill_status == 9" disable-transitions>异常订单</el-tag>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column fixed="right" label="操作" width="100">
                        <template slot-scope="scope">
                            {{-- <el-button @click="handleEdit(scope.row)" type="text" size="small">@{{scope.row.status == 2 ? '':'上架' }}
